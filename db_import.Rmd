...
dbSendQuery(con, "DROP TABLE IF EXISTS traps.locations;")

create_loc_q <- "
CREATE TABLE traps.locations (
id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
location text NOT NULL,
year integer NOT NULL,
ano_flate_id text NOT NULL
...
dbSendQuery(con, "CREATE INDEX ON traps.locations USING BTREE(location);")
dbSendQuery(con, "CREATE INDEX ON traps.locations USING BTREE(ano_flate_id);")
dbSendQuery(con, "CREATE INDEX ON traps.locations USING BTREE(year);")
```

```{r, eval = F}
dbWriteTable(con, 
             Id(schema = "traps", table = "locations"),
             loc,
             append = T)
```
...
landsskog_data
```
!!NOT DONE, continue formatting and make db-table
...
dbSendStatement(con, "CREATE INDEX ON landsskog.survey_data USING BTREE(treslag);")
